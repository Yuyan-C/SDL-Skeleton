...
class DataLayer(Dataset):
    ...
    def __init__(self, data_dir):
        ...
        self.fnLst = open("/network/scratch/y/yuyan.chen/SKLARGE/train.lst").readlines()
    ...
    def __getitem__(self, idx):
        ...
        self.image, self.flux, self.dilmask = self.loadsklarge(
            self.fnLst[idx].split("\n")[0]
        )
        return self.image, self.flux, self.dilmask

    def loadsklarge(self, imgidx):
        # load image and skeleton
        image = cv2.imread(f"{self.data_dir}/images/train/{imgidx}.jpg", 1)
        skeleton = sio.loadmat(f"{self.data_dir}/groundTruth/train/{imgidx}.mat")
        skeleton = (skeleton["scale"] > 0).astype(np.uint8)
        ...
        dst, labels = cv2.distanceTransformWithLabels(
            rev, cv2.DIST_L2, cv2.DIST_MASK_PRECISE, labelType=cv2.DIST_LABEL_PIXEL
        )
