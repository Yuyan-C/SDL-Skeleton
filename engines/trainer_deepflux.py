    ...
    def train(self):
        ...
        for step in range(self.args.resume_iter, self.args.max_step):
            ...
            for _ in range(self.args.iter_size):
                try:
                    image, flux, dilmask = next(dataiter)
                except StopIteration:
                    dataiter = iter(self.dataloader)
                    image, flux, dilmask = next(dataiter)

                image, flux, dilmask = (
                    image.cuda(self.args.gpu_id),
                    flux.cuda(self.args.gpu_id),
                    dilmask.cuda(self.args.gpu_id),
                )
                image, flux, dilmask = (
                    Variable(image),
                    Variable(flux),
            ...
            if (step + 1) % self.args.disp_interval == 0:
                ...
                print(
                    "{} iter={} loss={:<8.2f}".format(
                        timestr,
                        step + 1,
                        lossAcc / self.args.disp_interval / self.args.iter_size,
                    )
                )
                lossAcc = 0.0
            ...
            if (step + 1) % self.args.save_interval == 0:
                torch.save(
                    ...
                    "./weights/{}_{}.pth".format(self.args.network, step + 1),
                )

        torch.save(
            self.network.state_dict(),
            "./weights/{}.pth".format(self.args.network),
        )
